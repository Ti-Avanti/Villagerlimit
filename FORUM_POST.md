...
### 🎨 GUI 界面系统 ⭐ NEW!
- **统计 GUI** - 精美的个人统计界面，显示交易次数、消耗经验、常交易物品
- **排行榜 GUI** - 交易排行榜界面，前三名特殊显示（金/银/铜）
- **智能切换** - 玩家执行命令打开GUI，控制台显示文本
- **实时更新** - 数据实时同步，点击式操作
...
| `/vlstats [玩家]` | 查看交易统计（打开GUI） | `villagerlimit.stats` | ✅ 玩家列表 |
| `/vltop` | 查看交易排行榜（打开GUI） | `villagerlimit.top` | ✅ |
| `/vladmin <子命令>` | 管理命令 | `villagerlimit.admin` | ✅ reset/clear/info |
| `/vlperf [选项]` | 查看性能统计 | `villagerlimit.admin` | ✅ cache/memory/threads/tracker/cleanup |
...
villager-lifespan:
  ...
  days: 7  # 治愈获得的村民寿命（天）
  notify-enabled: true
  notify-range: 16  # 通知范围（0=全服）
  display-format: "§e剩余 {days}天 {hours}小时"
...
1. 下载插件jar文件 `Villagerlimit-2.1.2.jar`
...
**Q: GUI 界面如何打开？**
A: 玩家执行 `/vlstats` 或 `/vltop` 命令会自动打开对应的 GUI 界面。控制台执行则显示文本格式。

**Q: 排行榜变量如何使用？**
A: 在计分板、全息显示等支持 PlaceholderAPI 的地方使用 `%villagerlimit_top_1_name%` 等变量即可显示排行榜数据。

**Q: 村民寿命系统如何工作？**
A: 治愈僵尸村民获得的村民会自动设置寿命（可配置天数），头顶显示剩余时间，寿命到期后自动消失。
...
**Q: 支持哪些Minecraft版本？**
A: 支持 Minecraft 1.8 - 1.21.4，使用适配器模式实现版本兼容。
...
（此处可添加插件GUI界面、配置文件、游戏内效果、性能监控、排行榜等截图）
